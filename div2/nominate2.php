...
if(!isset($_GET['nominee'])){
    header('location: nominees22.php');
}

?>
    <br>&nbsp</br>
    <h3>Please, fill and submit the form below to nominate <?php echo "<big>$nominee</big> as " . $_SESSION['div2'] ; ?></h3>
...
if(($dup2->num_rows > 2) && ($rows2['dates'] == date("Y-m-d"))){
  ...
  exit();
    ...
    $sqln = "INSERT INTO nominations_c2(id, full_name, email, phone, dates) VALUES ('', '$yname', '$email1', '$phone', '$dates')";
    $conn->query($sqln);
