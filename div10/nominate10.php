...
if(!isset($_GET['nominee'])){
    header('location: nominees1010.php');
}

?>
    <br>&nbsp</br>
    <h3>Please, fill and submit the form below to nominate <?php echo "<big>$nominee</big> as " . $_SESSION['div10'] ; ?></h3>
...
if(($dup2->num_rows > 2) && ($rows2['dates'] == date("Y-m-d"))){
  ...
  exit();
    ...
    $sqln = "INSERT INTO nominations_c10(id, full_name, email, phone, dates) VALUES ('', '$yname', '$email1', '$phone', '$dates')";
    $conn->query($sqln);
